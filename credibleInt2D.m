function prob = credibleInt2D(d,alpha,plotFlag)
...
if iscell(d)
    ...
    for dIdx = 1:length(d)
        prob(dIdx) = credibleInt2D(d{dIdx},alpha);
        % prob(dIdx) = credibleInt2D(d{dIdx},alpha,normalApproxBW(dIdx),deltaXY);
...
[prob.map,xi,prob.bw] = ksdensity( bootstrp(nBoot,@mean,d) );
